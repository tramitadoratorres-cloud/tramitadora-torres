...
import { ACTIVIDAD_TIPO, ARCHIVO_CATEGORIA } from "@/lib/constants";
...
export async function subirReciboDerechosAction(
  casoId: string,
  _prevState: FormState,
  formData: FormData
): Promise<FormState> {
  ...
  await db.archivo.create({
    data: {
      casoId,
      categoria: ARCHIVO_CATEGORIA.DERECHOS_BANCO,
      nombre: guardado.nombre,
      rutaArchivo: guardado.rutaArchivo,
      mimeType: guardado.mimeType,
      tamano: guardado.tamano,
  ...
  await logActividad({
    ...
    userId: session.userId,
    tipo: ACTIVIDAD_TIPO.NOTA,
    descripcion: `${session.nombre} subió el comprobante de pago de derechos "${guardado.nombre}"`,
    visibleCliente: true,
  });
