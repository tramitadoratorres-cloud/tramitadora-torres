...
function Resumen({ cotizacion }: { cotizacion: ReturnType<typeof calcularCotizacion> }) {
  return <div className="mt-6"><div className="rounded-lg bg-ink/5 p-4"><div className="flex justify-between gap-4 text-sm"><span className="text-ink/65">Personas</span><strong>{cotizacion.personas} · {cotizacion.adultos} adulto{cotizacion.adultos === 1 ? "" : "s"}, {cotizacion.menores} menor{cotizacion.menores === 1 ? "" : "es"}</strong></div></div><div className="mt-5 space-y-3 border-b border-ink/10 pb-5">{cotizacion.lineas.map((linea, index) => <div key={`${linea.etiqueta}-${index}`} className="flex justify-between gap-3 text-sm"><span className="max-w-[70%] text-ink/70">{linea.cantidad > 1 ? `${linea.cantidad} × ` : ""}{linea.etiqueta}</span><strong className="whitespace-nowrap">{linea.moneda === "MXN" ? formatMXN(linea.cantidad * linea.monto) : formatUSD(linea.cantidad * linea.monto)}</strong></div>)}</div><div className="mt-5 space-y-2"><div className="flex justify-between text-sm"><span className="text-ink/65">Total honorarios</span><strong>{formatMXN(cotizacion.honorariosMXN)}</strong></div><div className="flex justify-between text-sm"><span className="text-ink/65">Derechos oficiales MXN</span><strong>{formatMXN(cotizacion.derechosMXN)}</strong></div>{cotizacion.derechosUSD > 0 && <div className="flex justify-between text-sm"><span className="text-ink/65">Derechos oficiales USD</span><strong>{formatUSD(cotizacion.derechosUSD)}</strong></div>}<div className="mt-4 flex justify-between rounded bg-navy-900 px-4 py-4 text-cream"><span className="font-serif text-lg">Total estimado MXN</span><strong className="font-serif text-2xl text-gold-bright">{formatMXN(cotizacion.totalMXN)}</strong></div>{cotizacion.derechosUSD > 0 && <p className="text-xs leading-relaxed text-ink/60">Los derechos en USD se pagan en dólares; no se convierten a MXN para evitar mostrar un tipo de cambio inexacto.</p>}</div><details className="mt-6"><summary className="cursor-pointer font-mono text-xs font-semibold uppercase tracking-wide text-navy-700">Requisitos para iniciar</summary><ul className="mt-3 space-y-2 text-sm text-ink/75">{cotizacion.requisitos.map((item) => <li key={item} className="pl-4 before:mr-2 before:text-gold before:content-['•']">{item}</li>)}</ul></details><p className="mt-5 text-xs leading-relaxed text-ink/55">{cotizacion.notas[0]} Las tarifas pueden modificarse por la dependencia sin previo aviso.</p></div>;
}
